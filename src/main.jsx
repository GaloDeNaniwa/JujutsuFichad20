...
function emptyCharacter(){ return {
  ...
  combat:{hpCurrent:null, hpTemp:0, peCurrent:null, peTemp:0, staminaCurrent:null, staminaTemp:0, soulCurrent:null, conditions:[], log:[]},
...
function Values({c,dispatch,stats}){ const trained=selectedTrainedSkills(c); const master=selectedMasterSkills(c); return <section className="sheetLike"><Panel title="Valores"><div className="valuesLayout"><div><h3>Testes e Perícias</h3><table><tbody>{trainableSkills().map(s=><tr key={s.id}><td>{s.name}</td><td>{ATTR_LABEL[s.attribute]}</td><td>{master.includes(s.id)?'Mestre':trained.includes(s.id)?'Treinado':'-'}</td><td><b>{signed(stats.skillMap[s.id]||0)}</b></td></tr>)}</tbody></table></div><div><h3>Testes de Resistência</h3><table><tbody>{RESISTANCES.map(([id,name])=><tr key={id}><td>{name}</td><td>{(c.choices.resistances||[]).includes(id)?(restringidoMasterResistances(c).includes(id)?'Mestre':'Treinado'):'-'}</td><td><b>{signed(stats.resMap?.[id]||0)}</b></td></tr>)}</tbody></table></div><div><h3>Atributos</h3><div className="attrOrbit">{ATTRS.map(([k,l,abbr])=><div key={k} className="orb"><span>{abbr}</span><b>{finalAttr(c,k)??0}</b><small>{signed(mod(finalAttr(c,k)))}</small></div>)}</div></div><div><h3>Defesas e Recursos</h3><Stat label="Defesa" value={stats.defense}/><Stat label="RD" value={stats.rd}/>{stats.cd!=null&&<Stat label="CD" value={stats.cd}/>}<Stat label="Atenção" value={stats.attention}/><Stat label="Iniciativa" value={signed(stats.initiative)}/><Stat label="Movimento" value={`${stats.movement}m`}/></div></div></Panel><Panel title="Registro Rápido">{c.inventory.items.filter(i=>i.equipped&&(i.type==='weapon'||i.type==='shield')).map(i=><WeaponCard key={i.instanceId} item={i} compact/> )}</Panel></section> }
...
function TocadoPorDeusPanel({c,dispatch}){
  ...
  return <Panel title="Novo Estilo da Sombra — Tocado por Deus (Homebrew)">
    ...
    <RuleBox title={TOCADO_POR_DEUS_STYLE.name} text={TOCADO_POR_DEUS_STYLE.originalText}/>
    ...
    <div className="cards">{TOCADO_POR_DEUS_TECHNIQUES.map(t=>{ const added=activeNames.includes(normText(t.name)); return <div key={t.id} className={added?'feature selected':'feature'}><h3>{t.name}</h3><p>{formatRuleText(t.originalText).slice(0,320)}{t.originalText.length>320?'...':''}</p><button disabled={added} onClick={()=>dispatch({type:'addPresetActive',preset:t})}>{added?'Já adicionada em Ativas':'Adicionar como Ativa (Técnica)'}</button></div>; })}</div>
    ...
    <div className="cards">{ESCOLHIDO_ARTIFACTS.map(a=>{ const owned=(c.inventory.items||[]).some(i=>i.name===a.name); return <div key={a.id} className={owned?'feature selected':'feature'}><h3>{a.name}</h3><p>{formatRuleText(a.originalText).slice(0,320)}...</p><button disabled={owned} onClick={()=>dispatch({type:'addItem',item:{...a,homebrew:true}})}>{owned?'Já no inventário':'Adicionar ao inventário'}</button></div>; })}</div>
...
CombatSheet = function CombatSheet({c,dispatch,stats}){ const [dmg,setDmg]=useState(''); const [heal,setHeal]=useState(''); const [pe,setPe]=useState(''); const [stam,setStam]=useState(''); const equipped=c.inventory.items.filter(i=>i.equipped); const hasStamina=restringidoIsSpec(c); return <section className="grid gap"><Panel title="Ficha / Combate"><div className="heroGrid"><div><h2>{c.name||'Personagem sem nome'}</h2><p>{origin(c)?.name||'Origem não escolhida'} · {specialization(c)?.name||'Especialização não escolhida'} · Nível {c.level}</p></div><Stat label="Defesa" value={stats.defense} icon={<Shield/>}/><Stat label="Atenção" value={stats.attention} icon={<Eye/>}/><Stat label="Movimento" value={`${stats.movement}m`} icon={<Swords/>}/>{stats.cd!=null&&<Stat label="CD" value={stats.cd} icon={<Zap/>}/>}{stats.rd>0&&<Stat label="RD" value={stats.rd}/>}</div><div className="resources"><Resource label="PV" current={c.combat.hpCurrent??stats.hpMax} max={stats.hpMax} temp={c.combat.hpTemp}/>{!hasStamina&&<Resource label="PE" current={c.combat.peCurrent??stats.peMax} max={stats.peMax} temp={c.combat.peTemp}/>}{hasStamina&&<Resource label="Estamina" current={c.combat.staminaCurrent??stats.staminaMax} max={stats.staminaMax} temp={c.combat.staminaTemp}/>}<Resource label="Alma" current={c.combat.soulCurrent??stats.soulMax} max={stats.soulMax} temp={0}/></div><div className="grid3"><Field label="Dano recebido"><input value={dmg} onChange={e=>setDmg(e.target.value)} /><button onClick={()=>{dispatch({type:'applyDamage',value:dmg});setDmg('')}}>Aplicar dano</button></Field><Field label="Cura recebida"><input value={heal} onChange={e=>setHeal(e.target.value)} /><button onClick={()=>{dispatch({type:'applyHeal',value:heal});setHeal('')}}>Aplicar cura</button></Field>{!hasStamina&&<Field label="Gasto de PE"><input value={pe} onChange={e=>setPe(e.target.value)} /><button onClick={()=>{dispatch({type:'spendPE',value:pe});setPe('')}}>Gastar</button></Field>}{hasStamina&&<Field label="Gasto de Estamina"><input value={stam} onChange={e=>setStam(e.target.value)} /><button onClick={()=>{dispatch({type:'spendStamina',value:stam});setStam('')}}>Gastar</button></Field>}</div>{hasStamina&&<div className="row"><button onClick={()=>dispatch({type:'restoreStamina',rest:'curto'})}>Descanso curto (metade da Estamina)</button><button onClick={()=>dispatch({type:'restoreStamina',rest:'longo'})}>Descanso longo (Estamina cheia)</button></div>}</Panel><Panel title="Ações rápidas e ataques"><div className="cards">{equipped.filter(i=>i.type==='weapon'||i.type==='shield').map(i=><WeaponCard key={i.instanceId} item={i} compact character={c}/>)}{equipped.length===0&&<p>Nenhum equipamento equipado.</p>}</div><div className="actionList improved">{rules.combatActions.map(a=><ActionCard key={a.id} a={a}/>)}</div></Panel><Panel title="Condições"><div className="conditionGrid">{rules.conditions.map(cond=><label key={cond.id} className={c.combat.conditions.includes(cond.id)?'condition active':'condition'}><input type="checkbox" checked={c.combat.conditions.includes(cond.id)} onChange={()=>dispatch({type:'toggleCondition',id:cond.id})}/><b>{cond.name}</b><Tooltip text={cond.originalText}/></label>)}</div></Panel><Panel title="Log de sessão"><div className="log">{c.combat.log.map(l=><div key={l.id}><span>{new Date(l.at).toLocaleTimeString()}</span>{l.label}</div>)}</div></Panel></section> }
...
function classFeatureChoiceLimit(c){ let n=Math.max(0, Number(c.level||1)-1); if(semTecnicaId(c.originId)||escolhidoId(c.originId)) n+=semTecnicaExtraClassFeatureLimit(c); return n; }
...
talentLimit = function talentLimit(c){
  let n=0; if(c.originId===ORIGIN.INATO) n+=1; if(semTecnicaId(c.originId)||escolhidoId(c.originId)) n+=semTecnicaChoiceTalentAptLimit(c); return n;
};
aptitudeLimit = function aptitudeLimit(c){
  ...
  if(semTecnicaId(c.originId)||escolhidoId(c.originId)) n+=semTecnicaChoiceTalentAptLimit(c);
  if((semTecnicaId(c.originId)||escolhidoId(c.originId)) && Number(c.level||1)>=4) n+=1;
  return n;
};
...
skillBonus = function skillBonus(c,s){ const av=finalAttr(c,s.attribute); const trained=selectedTrainedSkills(c).includes(s.id); const master=selectedMasterSkills(c).includes(s.id); const tb=trainingBonus(c.level); const lvlHalf=Math.floor(Number(c.level||1)/2); let extra=0; if(fetoId(c.originId)&&hasAnatomy(c,'olhos_sombrios')&&s.id==='percepcao') extra+=2; if(fetoId(c.originId)&&hasAnatomy(c,'bracos_extras')&&s.id==='prestidigitacao') extra+=2; if(fetoId(c.originId)&&hasAnatomy(c,'corpo_especializado')&&c.choices?.corpoEspecializadoSkill===s.id) extra+=2; extra+=restringidoVersatilidadeBonus(c); extra+=restringidoGiftAttrBonus(c,s.attribute); if(restringidoIsSpec(c)&&s.id==='percepcao'&&restringidoHasGift(c,'percepcao_aguçada')) extra+=3; if((semTecnicaId(c.originId)||escolhidoId(c.originId)) && empenhoSkills(c).includes(s.id)){ const tier=empenhoImplacavelTier(c); if(tier) extra+=tier.skillBonus; } return mod(av)+lvlHalf+(trained?tb:0)+(master?Math.floor(tb/2):0)+extra; };
function resistanceBonus(c,resId){ const attrKey=resistanceAttrKey(resId); const av=finalAttr(c,attrKey); const trained=(c.choices.resistances||[]).includes(resId); const master=restringidoMasterResistances(c).includes(resId); const tb=trainingBonus(c.level); const lvlHalf=Math.floor(Number(c.level||1)/2); let extra=restringidoGiftAttrBonus(c,attrKey); if(restringidoIsSpec(c)&&resId==='reflexes') extra+=restringidoEsquivaBonus(c.level); if(restringidoIsSpec(c)&&restringidoHasGift(c,'indulgente_a_feiticaria')){ const lvl=Number(c.level||1); if(resId==='will') extra+=1+(lvl>=10?1:0); if((resId==='fortitude'||resId==='reflexes') && lvl>=15) extra+=2; } if((semTecnicaId(c.originId)||escolhidoId(c.originId)) && empenhoAttackOrRes(c)===resId){ const tier=empenhoImplacavelTier(c); if(tier) extra+=tier.attackOrResBonus; } return mod(av)+lvlHalf+(trained?tb:0)+(master?Math.floor(tb/2):0)+extra; };
...
calc = function calc(c){
  ...
  const defense=10+dex+Math.floor(lvl/2)+(uniform?.defenseBonus||0)+accessoryBonus(c,'defense')+esquivaBonus+restringidoDefenseAttrBonus(c);
  const skillMap=Object.fromEntries(rules.skills.map(s=>[s.id, skillBonus(c,s)]));
  ...
  return {level:lvl,tb:trainingBonus(lvl),hpMax,peMax,staminaMax,soulMax:hpMax,movement,defense,attention,cd,initiative:dex+accessoryBonus(c,'initiative')+originInitiativeBonus(c),rd:(shield?.rd||0)+restringidoRd.physical,rdTechnique:restringidoRd.technique,skillMap,resMap};
};
levelTasks = function levelTasks(c,to){
