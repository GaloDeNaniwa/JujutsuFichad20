...
import combatActionPageIndex from './data/action-page-index.json';
import './styles.css';
...
function ChoiceCards({items,selected,onChange,limit=999,character=null,requirementCheck=null}){ const [q,setQ]=useState(''); const [modal,setModal]=useState(null); const filtered=items.filter(i=>(i.name+' '+(i.originalText||'')).toLowerCase().includes(q.toLowerCase())); const checkReq=requirementCheck||(i=>character?aptitudeRequirementStatus(character,i):{ok:true}); return <><Field label="Buscar"><input value={q} onChange={e=>setQ(e.target.value)}/></Field><div className="cards">{filtered.map(i=>{ const checked=selected.includes(i.id); const req=checkReq(i); const blocked=(!checked && selected.length>=limit)||(!checked&&!req.ok); return <div key={i.id} className={checked?'feature selected':'feature'}><div className="row"><h3>{i.name}</h3><input type="checkbox" checked={checked} disabled={blocked} onChange={e=>onChange(e.target.checked?[...selected,i.id]:selected.filter(x=>x!==i.id))}/><button className="q" onClick={()=>setModal(i)}>?</button></div>{getPrereqText(i)&&<small className={req.ok?'okText':'warn'}>Pré-requisito: {getPrereqText(i)}</small>}<p>{mechanicalPreview(i.originalText,320)}</p>{!req.ok&&<small className="warn">Bloqueado: falta {req.problems?.join(', ')}</small>}{!checked&&selected.length>=limit&&<small className="warn">Limite atingido.</small>}<div className="featureFooter"><BookPageButton pageNumber={bookPageIndex[i.id]} title={i.name}/></div></div>})}</div>{modal&&<ModalText title={modal.name} text={formatRuleText(modal.originalText||'')} onClose={()=>setModal(null)}/>}</> }
...
function ActionTypeCard({info}){ const [open,setOpen]=useState(false); return <div className="actionTypeCard"><b>{info.name}</b><button className="q" onClick={()=>setOpen(true)}>?</button>{open&&<ModalText title={info.name} text={formatRuleText(info.text||'')} onClose={()=>setOpen(false)}/>}</div> }
function ActionSubCard({a}){ const [open,setOpen]=useState(false); const txt=formatRuleText(a.text||''); const pageNumber=combatActionPageIndex[a.id]; return <div className="actionCard"><h3>{a.name}</h3><div className="textPreview actionText">{txt.slice(0,300)}{txt.length>300?'...':''}</div><div className="featureFooter">{txt.length>300&&<button onClick={()=>setOpen(true)}>Ver ação completa</button>}<BookPageButton pageNumber={pageNumber} title={a.name}/></div>{open&&<ModalText title={a.name} text={txt} onClose={()=>setOpen(false)}/>}</div> }
