...
const SPEC_SUMMARY = {
  lutador: {
    bonuses: ['PV: 12 + mod. Constituição no 1º nível; +1d10 (ou 6 fixo) + mod. Constituição por nível.', 'Treinado em Armas Simples, Marciais e Escudo Leve; TR de Fortitude ou Reflexos; 1 perícia de Ofício/Atletismo/Acrobacia + 3 livres.', '4 PE por nível. Atributo-chave: Força ou Destreza.', 'Corpo Treinado: ataque desarmado extra como Ação Bônus; dano desarmado 1d8, subindo até 1d12 nos níveis 5/9/13/17.'],
    playstyle: 'Linha de frente corpo a corpo, ágil e resistente — foco em socos, chutes e armas marciais.'
  },
  especialista_combate: {
    bonuses: ['PV: 12 + mod. Constituição no 1º nível; +1d10 (ou 6 fixo) + mod. Constituição por nível.', 'Treinado em todas as armas e escudos; TR de Fortitude ou Reflexos; 2 perícias de Ofício/Atletismo/Acrobacia + 3 livres.', '4 PE por nível. Atributo-chave: Força, Destreza ou Sabedoria.', 'Repertório do Especialista: escolhe 1 estilo de combate no 1º nível (Defensivo, Arremessador, Duelista...) que melhora em níveis altos.'],
  ...
  especialista_tecnica: {
    bonuses: ['PV: 10 + mod. Constituição no 1º nível; +1d8 (ou 5 fixo) + mod. Constituição por nível.', 'Treinado em Armas Simples e a Distância; TR de Astúcia ou Vontade; 2 perícias de Ofício/Feitiçaria/Ocultismo + 2 livres.', '6 PE por nível + mod. do atributo de técnica. Atributo-chave: Inteligência ou Sabedoria.', 'Domínio dos Fundamentos: 2 "Mudanças de Fundamento" no 1º nível (+1 no 12º), que alteram como seus Feitiços funcionam.'],
  ...
  controlador: {
    bonuses: ['PV: 10 + mod. Constituição no 1º nível; +1d8 (ou 5 fixo) + mod. Constituição por nível.', 'Treinado em Armas Simples e a Distância; TR de Astúcia ou Vontade; 1 perícia de Ofício/Percepção/Persuasão + 2 livres.', '5 PE por nível + mod. do atributo de técnica. Atributo-chave: Presença ou Sabedoria.', 'Treinamento em Controle: começa com 2 Invocações (mais em vários níveis) e +1 invocação ativa simultânea.'],
  ...
  suporte: {
    bonuses: ['PV: 10 + mod. Constituição no 1º nível; +1d8 (ou 5 fixo) + mod. Constituição por nível.', 'Treinado em Armas Simples e Escudos; TR de Astúcia ou Vontade; 2 perícias de Ofício/Medicina/Prestidigitação + 3 livres.', '5 PE por nível + mod. do atributo de técnica. Atributo-chave: Presença ou Sabedoria.', 'Suporte em Combate: cura por Ação Bônus (2d6 + Presença/Sabedoria, escalando até 6d10 no nível 16).'],
...
function SpecializationSkillPicker({c,dispatch,cfg,skillPool,skillMax}){
  ...
  const freeCandidates=skillPool.filter(s=>!baseIds.has(s.id));
  ...
  return <div className="skillPickerBlock">
    {baseCandidates.length>0 && <>
      <div className={baseChosen<cfg.skillBaseNeed?'notice warnNotice':'notice'}><b>Grupo obrigatório da especialização: {baseChosen}/{cfg.skillBaseNeed}</b><span> — escolha ao menos {cfg.skillBaseNeed} entre estas {baseCandidates.length} perícias específicas do livro. Cada uma conta aqui, não nas perícias livres abaixo.</span></div>
      <LimitedChoiceGrid title="Perícias do grupo obrigatório" items={baseCandidates.map(s=>({...s, originalText:(SKILL_HELP[s.id]?SKILL_HELP[s.id]+' ':'')+'Conta para o grupo obrigatório desta especialização.'}))} selected={selected} limit={skillMax} onChange={onChange}/>
      {selected.includes('oficio') && <OfficeSelector c={c} dispatch={dispatch}/>}
    </>}
    <div className="notice"><b>Perícias livres: {totalChosen}/{skillMax}</b><span> — qualquer perícia treinável, além das {cfg.skillBaseNeed} do grupo obrigatório. Extras por INT ou SAB: +{extraSkillSlots(c)}.</span></div>
...
function selectedTrainedSkills(c){ return [...new Set([...(c.choices.skills||[]), ...originFixedSkills(c)])]; };
