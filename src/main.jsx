...
function initialState(){
  ...
  try {
    ...
    return {...base, ...parsed, characters, activeCharacterId};
  } catch(e){
...
function reducer(state, action){
  ...
  switch(action.type){
    ...
    case 'update': return withChar(c=>{ const next={...c,[action.key]:action.value}; if(action.key==='specializationId'){ const cfg=specTrainingConfig(next); next.choices={...next.choices, resistances:[...new Set([...(cfg.resFixed||[]), ...(next.choices.resistances||[])])].slice(0,cfg.resMax)}; } return next; });
    ...
    case 'reset': localStorage.removeItem(STORAGE_KEY); return initialState();
...
function finalAttr(c,k){ const base=rollValue(c,c.attributes.assigned[k]); if(base==null) return null; const inc=(typeof attributeIncreaseMap==='function'?Number(attributeIncreaseMap(c)[k]||0):0); return base + Number(c.attributes.originBonuses[k]||0) + Number(c.attributes.temp[k]||0) + inc; }
...
function skillBonus(c,s){ const av=finalAttr(c,s.attribute); const trained=(c.choices.skills||[]).includes(s.id); const master=(c.choices.masterSkills||[]).includes(s.id); const tb=trainingBonus(c.level); return mod(av)+(master?tb*2:(trained?tb:0)); }
...
function extraSkillSlots(c){ return Math.max(0,mod(finalAttr(c,'intelligence'))) + Math.max(0,mod(finalAttr(c,'wisdom'))); }
...
function validation(c){
  ...
  const masterMax=c.level>=10?1:0;
  const invalidMasters=(c.choices.masterSkills||[]).filter(id=>!(c.choices.skills||[]).includes(id));
  ...
  add('originBonus','Resolver bônus da Origem',Object.keys(c.choices.originBonuses||{}).length>=2,'Criação Guiada','Ex.: +2 e +1 em atributos diferentes.');
  ...
  add('skills',`Escolher perícias treinadas (${(c.choices.skills||[]).length}/${skillMax})`,(c.choices.skills||[]).length===skillMax && invalidSkills.length===0 && baseChosen>=cfg.skillBaseNeed,'Criação Guiada', invalidSkills.length?`Inválidas: ${invalidSkills.map(skillName).join(', ')}`:`Precisa ${cfg.skillBaseNeed} do grupo da especialização; escolhidas ${baseChosen}. Bônus por Inteligência/Sabedoria: +${extraSkillSlots(c)}.`);
  add('masterSkill',`Perícia mestre (${(c.choices.masterSkills||[]).length}/${masterMax})`,c.level<10 || ((c.choices.masterSkills||[]).length===masterMax && invalidMasters.length===0),'Criação Guiada','No nível 10, escolha uma perícia treinada para se tornar mestre.');
...
function usePersist(state){ useEffect(()=>{ localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); },[state]); }
...
validation = function validation(c){
  ...
  add('cd','Escolher atributo de CD',!!c.cdAttribute,'Criação Guiada');
  add('res',`Escolher resistências permitidas (${(c.choices.resistances||[]).length}/${resMax})`,(c.choices.resistances||[]).length===resMax && invalidRes.length===0,'Criação Guiada', invalidRes.length?`Inválidas: ${invalidRes.map(resistanceName).join(', ')}`:cfg.notes);
  add('skills',`Escolher perícias treinadas (${sCount}/${skillMax})`,sCount===skillMax && invalidSkills.length===0 && baseChosen>=cfg.skillBaseNeed,'Criação Guiada', invalidSkills.length?`Inválidas: ${invalidSkills.map(skillName).join(', ')}`:`Grupo obrigatório: ${baseChosen}/${cfg.skillBaseNeed}. Bônus por INT/SAB: +${extraSkillSlots(c)}. Ofício pode ter mais de uma especialização.`);
  add('masterSkill',`Perícia mestre (${(c.choices.masterSkills||[]).length}/${masterMax})`,c.level<10 || ((c.choices.masterSkills||[]).length===masterMax && invalidMasters.length===0),'Criação Guiada','No nível 10, escolha uma perícia treinada para se tornar mestre.');
  if((c.choices.skills||[]).includes('oficio')) add('oficio-detail','Informar os Ofícios treinados.',oficioLines(c).length>=1,'Criação Guiada','Informe um por linha. Ex.: Ferreiro, Alfaiate, Canalizador.');
  add('talents',`Talentos dentro do limite (${(c.choices.talents||[]).length}/${talentMax})`,(c.choices.talents||[]).length<=talentMax,'Perfil Mundano','Talentos ficam bloqueados quando o limite é atingido.');
...
function AttributeIncreaseManager({c,dispatch}){
  ...
  return <Panel title={`Pontos de atributo por nível: ${spent}/${total}`}><p className="muted">A cada 4 níveis, o personagem recebe +2 pontos de atributo. Estes pontos são permanentes e entram no cálculo automático.</p>{total===0?<div className="notice">Nenhum ponto de atributo por nível liberado ainda. O primeiro ganho ocorre no nível 4.</div>:<><div className={pending>0?'notice warnNotice':'notice'}><b>{pending>0?`Faltam distribuir ${pending} ponto(s).`:'Todos os pontos de atributo por nível foram distribuídos.'}</b></div><div className="attrIncreaseGrid">{ATTRS.map(([k,label])=>{ const current=Number(attributeIncreaseMap(c)[k]||0); const canAdd=pending>0; return <div key={k} className="attrIncCard"><b>{label}</b><strong>+{current}</strong><div className="row"><button disabled={current<=0} onClick={()=>dispatch({type:'choiceObject',key:'attributeIncreases',id:k,value:Math.max(0,current-1)})}>−</button><button disabled={!canAdd} onClick={()=>dispatch({type:'choiceObject',key:'attributeIncreases',id:k,value:current+1})}>+</button></div></div>})}</div></>}</Panel>
}
...
function AttributeRollPanel({c,dispatch}){
  ...
  return <Panel title="2. Atributos por rolagem" help="Role seis resultados e distribua livremente. Depois some bônus de Origem e pontos permanentes de nível.">
    <div className="row wrap"><button className="gold" onClick={()=>dispatch({type:'roll'})}>{rs?'Rerolar atributos':'Rolar atributos'}</button><span className="muted">4d6, descarta o menor. Rerrolagens gastam crédito após a gratuita.</span></div>
    {rs&&<><div className="rolls improvedRolls">{rs.rolls.map(r=><span key={r.id} className={usedRollIds.includes(r.id)?'used':''}>{r.total}<small>{r.dice.join(', ')} − {r.dropped}</small></span>)}</div>
    <div className="attrGrid improvedAttrs compactAttrs">{ATTRS.map(([k,label,abbr])=>{
      const val=finalAttr(c,k); return <div className="attrCard" key={k}><h3>{label}<Tooltip text={ATTRIBUTE_HELP[k]}/></h3><select value={c.attributes.assigned[k]||''} onChange={e=>dispatch({type:'assignAttr',attr:k,rollId:e.target.value})}><option value="">—</option>{rs.rolls.map(r=><option key={r.id} value={r.id} disabled={usedRollIds.includes(r.id)&&c.attributes.assigned[k]!==r.id}>{r.total}</option>)}</select><div className="attrResult"><b>{val??'—'}</b><strong>{signed(mod(val))}</strong><small>{abbr}</small></div><small className="muted">Origem +{Number(c.attributes.originBonuses[k]||0)} · Nível +{Number(attributeIncreaseMap(c)[k]||0)}</small></div>})}</div></>}
  </Panel>
}
...
Mundane = function Mundane({c,dispatch}){ 
  ...
  return <section className="grid gap mundanePage"><Panel title="Perfil Mundano"><div className="grid3"><Stat label="Especialização" value={sp?.name||'—'}/><Stat label="Origem" value={org?.name||'—'}/><Stat label="Exaustão" value={c.mundaneProfile?.exhaustionLevel||0}/></div><div className="notice"><b>Maestrias/Treinamentos:</b> {cfg.masteryText}</div></Panel>
