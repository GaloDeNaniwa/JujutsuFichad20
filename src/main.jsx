...
function TaskList({tasks=[],dispatch}){
  ...
  return <div className="taskList">{list.map((t,i)=>{
    ...
    return <div key={t?.id || i} className={ok ? 'good' : 'bad'}>
      ...
      {!ok && t?.tab && dispatch && <button onClick={()=>dispatch({type:'tab',tab:t.tab})}>Ir</button>}
    </div>;
  })}</div>;
}
